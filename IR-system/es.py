...
indexName = "cs172final6" + "2052"#str(time.time())[-4:] # avoid repeated indexName
...
query = input("Search: ")
while query.lower() != "q":
    print('Query keywords: %s \n' % query)
    response = esConn.search(index=indexName, body=MyUtils.generteDsl(query))
    MyUtils.printResult(response)
    query = input("Search (Enter 'q' to exit):")
