...
def printResult(response):
    for i in range(len(response["hits"]["hits"])):
        ...
        print("Result# :", i+1 , " | Score:  %.3f"%temp["_score"], " | Title: ", temp['_source']['news_title'].replace("â","").replace("",""))
        if temp['_source']["text"]:
            print ("Snippet Result:  ", MyAbstract.generateSummary(temp['_source']["text"], 2))
        print('Link: %s \n' % temp['_source']['link'])
